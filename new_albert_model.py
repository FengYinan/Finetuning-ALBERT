...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.embeddings = AlbertEmbeddings(config).to(DEVICE)
        self.encoder = AlbertTransformer(config).to(DEVICE)
        self.pooler = nn.Linear(config.hidden_size, config.hidden_size).to(DEVICE)
        self.pooler_activation = nn.Tanh().to(DEVICE)
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels).to(DEVICE)

        self.whole_time=0
        self.last_time=0
        self.last_comu_time = 0
    ...
    def pow_loss(self,logits, labed):
        resutl = torch.log(torch.pow(logits,2)/torch.sum(torch.pow(logits,2), dim=-1, keepdim=True))
